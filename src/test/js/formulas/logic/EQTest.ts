...
import { EQ } from "../../../../main/js/formulas/logic/EQ";
...
describe("EQ", function () {
  it("should do same type equality", function () {
    assert.equal(EQ.SELF.run(null, 10.0, 10.0), true);
    assert.equal(EQ.SELF.run(null, 10.0, 0.0), false);
    assert.equal(EQ.SELF.run(null, "Same", "Same"), true);
    assert.equal(EQ.SELF.run(null, "Same", "Different"), false);
    assert.equal(EQ.SELF.run(null, true, true), true);
    assert.equal(EQ.SELF.run(null, true, false), false);
    assert.equal(EQ.SELF.run(null, false, false), true);
  });

  it("should do pass-through errors", function () {
    assert.equal(
      (EQ.SELF.run(null, 4.4444, new ValueException()) as F7Exception).name,
  ...
  it("should use lookup", function () {
    ...
    const F = new EQ(lookup as LookupFunction, collateralLookup as CollateralLookupFunction);
  ...
  it("should handle grids", function () {
    ...
    assert.deepEqual(EQ.SELF.run(null, one, two), true);
  });

  it("should return error when argument lengths are wrong", function () {
    assert.deepEqual((EQ.SELF.run(null, "Too few") as NAException).name, F7ExceptionName.NA);
    assert.deepEqual(
      (EQ.SELF.run(null, "A", "B", "Too many") as NAException).name,
      F7ExceptionName.NA
    );
  });
});
