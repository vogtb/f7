...
import { UNARY_PERCENT } from "../../../../main/js/formulas/math/UNARY_PERCENT";
import { Grid } from "../../../../main/js/models/common/Grid";
import { CollateralLookupFunction, LookupFunction } from "../../../../main/js/models/common/Types";
import { CommonModels } from "../../testutils/CommonModels";

describe("UNARY_PERCENT", function () {
  it("should do normal operations", function () {
    assert.deepEqual(UNARY_PERCENT.SELF.run(null, 10.0), 0.1);
    assert.deepEqual(UNARY_PERCENT.SELF.run(null, -10.0), -0.1);
    assert.deepEqual(UNARY_PERCENT.SELF.run(null, 0.0), 0.0);
    assert.deepEqual(UNARY_PERCENT.SELF.run(null, 8278.28687), 82.7828687);
  });

  it("should do string conversion", function () {
    assert.deepEqual(UNARY_PERCENT.SELF.run(null, "10"), 0.1);
  });

  it("should do pass-through errors", function () {
    assert.deepEqual(
      (UNARY_PERCENT.SELF.run(null, new ValueException()) as F7Exception).name,
  ...
  it("should use lookup", function () {
    ...
    assert.deepEqual(F.run(CommonModels.A1, CommonModels.M22_RANGE), 0.02);
  ...
  it("should handle grids", function () {
    ...
    assert.deepEqual(UNARY_PERCENT.SELF.run(null, one), 0.02);
  });

  it("should return error when argument lengths are wrong", function () {
    assert.deepEqual(
      (UNARY_PERCENT.SELF.run(null, "A", "Too many") as NAException).name,
      F7ExceptionName.NA
    );
  });
});
