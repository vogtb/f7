...
import { UPLUS } from "../../../../main/js/formulas/math/UPLUS";
import { Grid } from "../../../../main/js/models/common/Grid";
import { CollateralLookupFunction, LookupFunction } from "../../../../main/js/models/common/Types";
import { CommonModels } from "../../testutils/CommonModels";

describe("UPLUS", function () {
  it("should do normal operations", function () {
    assert.deepEqual(UPLUS.SELF.run(null, 10), 10);
    assert.deepEqual(UPLUS.SELF.run(null, 0), 0);
    assert.deepEqual(UPLUS.SELF.run(null, 8278.28687), 8278.28687);
  });
  ...
  it("should do pass-through errors", function () {
    assert.deepEqual(
      (UPLUS.SELF.run(null, new ValueException()) as F7Exception).name,
  ...
  it("should use lookup", function () {
    ...
    const F = new UPLUS(lookup as LookupFunction, collateralLookup as CollateralLookupFunction);
  ...
  it("should handle grids", function () {
    ...
    assert.deepEqual(UPLUS.SELF.run(null, one), 2);
  });

  it("should return error when argument lengths are wrong", function () {
    assert.deepEqual(
      (UPLUS.SELF.run(null, "A", "Too many") as NAException).name,
      F7ExceptionName.NA
    );
  });
});
