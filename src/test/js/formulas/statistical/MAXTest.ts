...
import { MAX } from "../../../../main/js/formulas/statistical/MAX";
...
describe("MAX", function () {
  ...
  it("should do min minimum value for numbers", function () {
    assert.deepEqual(MAX.SELF.run(null, 1, 2, 3, 4, 5, 6, 7, 8), 8);
    assert.deepEqual(MAX.SELF.run(null, 1, 2, 3, 4, 5, 10, 7, 8), 10);
    assert.deepEqual(MAX.SELF.run(null, 0), 0);
    assert.deepEqual(MAX.SELF.run(null, 2984723.99382), 2984723.99382);
    assert.deepEqual(MAX.SELF.run(null, 1, 2, -1000), 2);
  });

  it("should return VALUE error when encountering text values outside ranges", function () {
    assertF7ExceptionByName(MAX.SELF.run(null, 1, 2, "Bad"), F7ExceptionName.VALUE);
  });

  it("should ignore text values inside of grids", function () {
    assert.deepEqual(MAX.SELF.run(null, GRID_WITH_TEXT_VALUE), -10);
  });

  it("should return 0 when the resulting filter range is all null", function () {
    assert.deepEqual(MAX.SELF.run(null, Grid.builder().build()), 0);
  });

  it("should consider booleans", function () {
    assert.deepEqual(MAX.SELF.run(null, true, false, -10), 1);
  });

  it("should ignore blank cells (null values)", function () {
    assert.deepEqual(MAX.SELF.run(null, GRID_WITH_BLANK_VALUES), 4);
  });

  it("should use lookup function for range queries", function () {
    const lookup = stub();
    const collateralLookup = stub();
    const F = new MAX(lookup as LookupFunction, collateralLookup as CollateralLookupFunction);
  ...
  it("should return error when argument lengths are wrong", function () {
    assert.deepEqual((MAX.SELF.run(null) as NAException).name, F7ExceptionName.NA);
  });
});
