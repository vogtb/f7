...
import { MAXA } from "../../../../main/js/formulas/statistical/MAXA";
...
describe("MAXA", function () {
  ...
  it("should do min value for numbers", function () {
    assert.deepEqual(MAXA.SELF.run(null, 1, 2, 3, 4, 5, 6, 7, 8), 8);
    assert.deepEqual(MAXA.SELF.run(null, 1, 2, 3, 4, 5, 10, 7, 8), 10);
    assert.deepEqual(MAXA.SELF.run(null, 0), 0);
    assert.deepEqual(MAXA.SELF.run(null, 2984723.99382), 2984723.99382);
    assert.deepEqual(MAXA.SELF.run(null, 1, -2, 1000), 1000);
  });

  it("should return VALUE error when encountering non-coercable text values outside ranges", function () {
    assertF7ExceptionByName(MAXA.SELF.run(null, 1, 2, "Bad"), F7ExceptionName.VALUE);
    assert.deepEqual(MAXA.SELF.run(null, 0, "10"), 10);
  });

  it("should convert to text values to zero inside of grids", function () {
    assert.deepEqual(MAXA.SELF.run(null, GRID_WITH_TEXT_VALUE), 0);
  });

  it("should return 0 when the resulting filter range is all null", function () {
    assert.deepEqual(MAXA.SELF.run(null, Grid.builder().build()), 0);
  });

  it("should consider booleans", function () {
    assert.deepEqual(MAXA.SELF.run(null, true, false, -10), 1);
  });

  it("should throw errors inside and outside of grids", function () {
    assertF7ExceptionByName(MAXA.SELF.run(null, 1, 2, new RefException()), F7ExceptionName.REF);
    assertF7ExceptionByName(
      MAXA.SELF.run(null, Grid.builder().add(0, 0, new RefException()).build()),
      F7ExceptionName.REF
    );
  });

  it("should ignore blank cells (null values)", function () {
    assert.deepEqual(MAXA.SELF.run(null, GRID_WITH_BLANK_VALUES), 4);
  });

  it("should use lookup function for range queries", function () {
    const lookup = stub();
    const collateralLookup = stub();
    const F = new MAXA(lookup as LookupFunction, collateralLookup as CollateralLookupFunction);
    lookup.withArgs(CommonModels.M22_RANGE).returns(GRID);
    assert.deepEqual(F.run(CommonModels.A1, CommonModels.M22_RANGE), 131289731);
  ...
  it("should return error when argument lengths are wrong", function () {
    assert.deepEqual((MAXA.SELF.run(null) as NAException).name, F7ExceptionName.NA);
  });
});
