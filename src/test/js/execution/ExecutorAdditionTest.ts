...
describe("Executor.execute - Addition", function () {
  it("should work with numbers", function () {
    runner().addCell("Alpha", "A1", "= 10 + 10").addExpectedValue("Alpha", "A1", 20).run();
    runner().addCell("Alpha", "A1", "= 0 + 0").addExpectedValue("Alpha", "A1", 0).run();
    runner().addCell("Alpha", "A1", "= 10 + -10").addExpectedValue("Alpha", "A1", 0).run();
    runner().addCell("Alpha", "A1", "= -10 + -10").addExpectedValue("Alpha", "A1", -20).run();
    runner()
      .addCell("Alpha", "A1", "= 1e10 + 1.1")
      .addExpectedValue("Alpha", "A1", 1.00000000011e10)
      .run();
  });

  it("should work with booleans", function () {
    runner().addCell("Alpha", "A1", "= 9 + TRUE").addExpectedValue("Alpha", "A1", 10).run();
    runner().addCell("Alpha", "A1", "= 9 + FALSE").addExpectedValue("Alpha", "A1", 9).run();
    runner().addCell("Alpha", "A1", "= 9 + -TRUE").addExpectedValue("Alpha", "A1", 8).run();
    runner().addCell("Alpha", "A1", "= 9 + -FALSE").addExpectedValue("Alpha", "A1", 9).run();
  });

  it("should work with strings", function () {
    runner().addCell("Alpha", "A1", '= 9 + "10"').addExpectedValue("Alpha", "A1", 19).run();
    runner()
      .addCell("Alpha", "A1", '= 9 + "1e10"')
      .addExpectedValue("Alpha", "A1", 1.0000000009e10)
      .run();
    runner().addCell("Alpha", "A1", '= 9 + "0"').addExpectedValue("Alpha", "A1", 9).run();
  });
  ...
  it("should work with blank values", function () {
    runner().addCell("Alpha", "A1", "= 2 + M44").addExpectedValue("Alpha", "A1", 2).run();
  });
