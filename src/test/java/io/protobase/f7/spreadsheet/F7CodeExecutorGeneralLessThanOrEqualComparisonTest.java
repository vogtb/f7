...
public class F7CodeExecutorGeneralLessThanOrEqualComparisonTest extends TestF7CodeExecutor {
  @Test
  public void test_Number() {
    assertThat(run("= 1 <= 1")).isEqualTo(true);
    assertThat(run("= 1 <= 2")).isEqualTo(true);
    assertThat(run("= -10 <= 0")).isEqualTo(true);
    assertThat(run("= 2 <= 1")).isEqualTo(false);
    assertThat(run("= 0 <= -10")).isEqualTo(false);
  ...
  public void test_StringToBoolean() {
    ...
    assertThat(run("= \"Anything\" <= FALSE")).isEqualTo(true);
  }
